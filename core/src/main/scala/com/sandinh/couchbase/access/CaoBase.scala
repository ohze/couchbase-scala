...
abstract class CaoBase[T, U, D <: Document[U]: ClassTag](bucket: ScalaBucket) {
  ...
  protected def createDoc(id: String, expiry: Int, content: U, cas: Long = 0L): D

  final def get(id: String): Future[T] = bucket.get[D](id).map(d => reads(d.content))

  final def getWithCAS(id: String): Future[DocumentCAS] = bucket.get[D](id).map(d => (reads(d.content), d.cas()))

  def getOrElse(id: String)(default: => T): Future[T] = get(id).recover {
    case _: DocumentDoesNotExistException => default
  }

  def getOrElseWithCAS(id: String)(default: => T): Future[DocumentCAS] = getWithCAS(id).recover {
    case _: DocumentDoesNotExistException => (default, -1)
  }
  ...
  final def update(id: String, t: T, cas: Long = 0): Future[D] = bucket.replace(createDoc(id, expiry(), writes(t), cas))
