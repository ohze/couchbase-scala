...
import com.couchbase.client.scala.codec.{LegacyTranscoder, RawJsonTranscoder}
...
class CompatStringSpec extends GuiceSpecBase {
  ...
  def stringSet(idSuffix: Int) =
    cb.bk1.upsert(id + idSuffix, s).map(_ => s)
  def stringGet(idSuffix: Int) =
    cb.bk1.getT[String](id + idSuffix)
  ...
  def jsonStringSet(idSuffix: Int) =
    cb.bk1
      .upsert(
        id + idSuffix,
        s,
        UpsertOptions().transcoder(LegacyTranscoder.Instance)
      )
      .map(_ => s)
  def jsonStringGet(idSuffix: Int) =
    cb.bk1
      .get(id + idSuffix, GetOptions().transcoder(LegacyTranscoder.Instance))
      .map(_.contentAs[String])
